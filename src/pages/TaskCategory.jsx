...
const TaskCategory = () => {
  ...
  const categories = [
    { id: 'daily', name: 'Daily Tasks', color: '#FF5C5C', description: 'Regular daily activities' },
    { id: 'project', name: 'Project Work', color: '#4CAF50', description: 'Project related tasks' },
    { id: 'personal', name: 'Personal', color: '#2196F3', description: 'Personal tasks and goals' },
    { id: 'urgent', name: 'Urgent', color: '#FF9800', description: 'High priority tasks' }
  ]
  ...
  // Initialize loading state with a shorter timeout
  useEffect(() => {
  ...
  // Initialize onboarding tour with console logs for debugging
  useEffect(() => {
    const initTour = async () => {
      console.log('Tour initialization started')
      console.log('Current user:', currentUser)
      console.log('Loading state:', loading)
      
      if (currentUser && !loading) {
        console.log('Attempting to start categories tour')
        try {
          await OnboardingService.checkAndStartTour(currentUser.uid, 'categories')
          console.log('Tour initialization completed')
        } catch (error) {
